...
export async function GET() {
  try {
    const [settingsResult, statsResult, topProducts, topCategories] = await Promise.all([
      turso.execute('SELECT * FROM shop_settings WHERE id = ?', ['main']),
      ...
      FROM orders`),
      turso.execute(`SELECT oi.product_name, SUM(oi.quantity) as sold, SUM(oi.price * oi.quantity) as revenue
        FROM order_items oi GROUP BY oi.product_id ORDER BY sold DESC LIMIT 5`),
      turso.execute(`SELECT c.name, c.product_count, COUNT(oi.id) as sold
        FROM categories c LEFT JOIN products p ON c.id = p.category_id 
        LEFT JOIN order_items oi ON p.id = oi.product_id 
        GROUP BY c.id ORDER BY sold DESC LIMIT 5`),
    ]);

    const revenueByMonth = await turso.execute(`
    ...
    const ordersByWeek = await turso.execute(`
